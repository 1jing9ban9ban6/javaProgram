...
import com.jing.weiming.dao.IOrderDao;
...
public class OrderServiceImpl implements IOrderService{
    ...
    public Map<String, Object> createOrder(String sku_id, String user_id){
        ...
        if (policy!=null&&!policy.equals("")){
            ...
            String now = restTemplate.getForObject("http://weiming-time-server/getTime", String.class);
    ...
    @Override
    public Map<String, Object> ms(String sku_id) {
        Map<String, Object> resultMap = new HashMap<>();
//        String lock = "key"+sku_id;
//        String userId  = UUID.randomUUID().toString();
//        try {
            // 分布式锁
//            Boolean key = stringRedisTemplate.opsForValue().setIfAbsent(lock, "userId", 20, TimeUnit.SECONDS);
//            if (!key){
//                resultMap.put("result", false);
//                resultMap.put("msg", "手慢了");
//                return resultMap;
//            }
            //1、从redis取政策
            String policy_num = stringRedisTemplate.opsForValue().get("LIMIT_POLICY_"+sku_id);
            //4、redis计数器
            //+1+1+1  --999 +1 +1
            long limitQuanty = Long.parseLong(policy_num);
            if (stringRedisTemplate.opsForValue().increment("SKU_QUANTY_"+sku_id, 1)<=limitQuanty) {
                stringRedisTemplate.opsForValue().increment("kc_"+sku_id, -1);
            } else {
                //6、没通过计数器的，返回错误信息，提示商品已经售完
                resultMap.put("result", false);
...
//        finally {
//            if (userId.equals(stringRedisTemplate.opsForValue().get(lock))){
//                // 释放锁
